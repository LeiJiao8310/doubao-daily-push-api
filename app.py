...
def get_tenant_access_token() -> str:
    ...
    if not APP_SECRET:
        raise RuntimeError("LARK_APP_SECRET is not configured. Put it in KMS/sensitive config/env, not in code.")
...
def get_base_token_from_wiki() -> str:
    ...
    data = lark_get(f"/wiki/v2/spaces/get_node", token, params={"token": WIKI_TOKEN})
...
def normalize_date(value: Any) -> Optional[str]:
    ...
    if isinstance(value, str):
        # Support YYYY-MM-DD or datetime string prefix.
        return value[:10]
    if isinstance(value, (int, float)):
        # Lark date fields are commonly milliseconds timestamp.
        ts = int(value)
